...
(function (global) {
  ...
  function load() {
    ...
    return { version: 1, scores: {}, plays: 0, players: {} };
  }
  ...
  var FDB = {
    ...
    submit: function (gameId, username, score, mode) {
      if (this.isBlocked(username)) return -1;
      var db = load();
      if (!db.scores[gameId]) db.scores[gameId] = [];
      db.scores[gameId].push({
        name: String(username || 'PLAYER').toUpperCase().slice(0, 12),
        score: score,
        date: new Date().toISOString().slice(0, 10)
      });
      var top = this.top(gameId, 10, mode);
      db.scores[gameId] = top;
      db.plays = (db.plays || 0) + 1;
      var key = String(username || 'PLAYER').toUpperCase().slice(0, 12);
      db.players[key] = (db.players[key] || 0) + 1;
      save(db);
      // posisi pemain ini
      for (var i = 0; i < top.length; i++) {
        if (top[i].name === key && top[i].score === score) return i + 1;
      }
      return -1;
    },
    ...
    stats: function () {
      ...
      var entries = 0;
      for (var g in db.scores) entries += db.scores[g].length;
      return { plays: db.plays || 0, players: players, entries: entries };
    },
